...
class Vismapay {
		...
		.then(data => {
			if (data.result !== undefined) {
				if (data.result === 0) {
					...
					return Promise.reject(new VismapayError('Error', 6, data));
				}
			} else {
				return Promise.reject(new VismapayError('Malformed response from Visma Pay API', 1));
			}
		});
	}
	...
	createCharge (charge) {
		if (this.apiKey == '' || this.privateKey == '') {
			return Promise.reject(new VismapayError('Private key or api key not set', 2));
		}

		if(!charge || !charge.amount || !charge.currency || !charge.order_number || !charge.payment_method) {
			return Promise.reject(new VismapayError('createCharge: Invalid parameters', 3));
		}
	...
	checkStatusWithToken (token) {
		...
		if(!token) {
			return Promise.reject(new VismapayError('checkStatusWithToken: token missing', 3));
		}
	...
	checkStatusWithOrderNumber (orderNumber) {
		...
		if(!orderNumber) {
			return Promise.reject(new VismapayError('checkStatusWithOrderNumber: order number missing', 3));
		}
	...
	capture (orderNumber) {
		if(!orderNumber) {
			return Promise.reject(new VismapayError('capture: order number missing', 3));
		}
	...
	cancel (orderNumber) {
		if(!orderNumber) {
			return Promise.reject(new VismapayError('cancel: order number missing', 3));
		}
	...
	getCardToken (cardToken) {
		if(!cardToken) {
			return Promise.reject(new VismapayError('getCardToken: card token missing', 3));
		}
	...
	deleteCardToken (cardToken) {
		if(!cardToken) {
			return Promise.reject(new VismapayError('deleteCardToken: card token missing', 3));
		}
		const bodyJson = {
			'version': encodeURIComponent(this.apiVersion),
			'api_key': encodeURIComponent(this.apiKey),
	...
	checkReturn (params) {
		if('RETURN_CODE' in params && 'AUTHCODE' in params && 'ORDER_NUMBER' in params) {
			...
			return Promise.reject(new VismapayError('checkReturn: MAC check failed', 5));
		}

		return Promise.reject(new VismapayError('checkReturn: Invalid parameters', 3));
	}
