...
class Vismapay {
	...
	async checkReturn (params) {
		if('RETURN_CODE' in params && 'AUTHCODE' in params && 'ORDER_NUMBER' in params) {
			...
			const expected = Buffer.from(calculatedMac, 'utf-8');
			const provided = Buffer.from(String(params.AUTHCODE), 'utf-8');
	...
	async getMerchantPaymentMethods (currency, channelSubMerchantIds) {
		const bodyJson = {
		...
		if (Array.isArray(channelSubMerchantIds) && channelSubMerchantIds.length > 0) {
			bodyJson.channel_sub_merchant_ids = channelSubMerchantIds;
		}
